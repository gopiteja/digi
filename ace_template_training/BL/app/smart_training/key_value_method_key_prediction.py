...
def kv_keywords_prediction(values, file_keywords):
    ...
    file_keywords_left = ocrDataLocal_special(value_scope['top'], value_scope['left'] - 100, value_scope['right'],
                                              value_scope['bottom'], file_keywords)

    # getting keyword top to value
    file_keywords_top = ocrDataLocal_special(value_scope['top'] - 20, value_scope['left'], value_scope['right'],
                                             value_scope['bottom'], file_keywords)

    # getting keyword aronud value
    file_keywords_key = ocrDataLocal_special(value_scope['top'] - 50, value_scope['left'] - 100, value_scope['right'],
                                             value_scope['bottom'], file_keywords)
...
def get_local_ocr_for_orientation(ocr_data, key_orientation, keyword_scope):
    ...
    if key_orientation == 'left':
        values = ocrDataLocal_special(keyword_scope['top'],
                                      keyword_scope['right'],
                                      keyword_scope['right'] + parameters['keyword_predict_right_ocr_boundary'],
                                      keyword_scope['bottom'],
                                      ocr_data)

    elif key_orientation == 'right':
        values = ocrDataLocal_special(keyword_scope['top'],
                                      keyword_scope['left'] - parameters['keyword_predict_left_ocr_boundary'],
                                      keyword_scope['left'],
                                      keyword_scope['bottom'],
                                      ocr_data)

    elif key_orientation == 'top':
        values = ocrDataLocal_special(keyword_scope['bottom'],
                                      keyword_scope['left'] - parameters['keyword_predict_left_ocr_boundary'],
                                      keyword_scope['right'] + parameters['keyword_predict_right_ocr_boundary'],
                                      keyword_scope['bottom'] + parameters['keyword_predict_bottom_ocr_boundary'],
                                      ocr_data)

    elif key_orientation == 'bottom':
        values = ocrDataLocal_special(keyword_scope['top'] - parameters['keyword_predict_top_ocr_boundary'],
                                      keyword_scope['left'] - parameters['keyword_predict_left_ocr_boundary'],
                                      keyword_scope['right'] + parameters['keyword_predict_right_ocr_boundary'],
                                      keyword_scope['top'], ocr_data)
    ...
    if file_keywords:
        ...
        if field_orientaion_dict:
            for key_orientation in field_orientaion_dict:
                ...
                closest_values = find_closest_mate(file_keywords[0], value_orientation, no_of_mate=1,
        ...
        if not closest_values:
            ...
            values_key = ocrDataLocal_special(keyword_scope['bottom'], keyword_scope['right'],
                                              keyword_scope['right'] + 200, keyword_scope['bottom'] + 50, values)
            values_key = remove_dangling_special_character(values_key)

            # get the keyword closet in left
            closest_values = find_closest_mate(file_keywords[0], values_left, no_of_mate=1,
                                               field_validation=field_validation)

            # get the keyword closet in top
            if not closest_values:
                closest_values = find_closest_mate(file_keywords[0], values_top, no_of_mate=1,
                                                   field_validation=field_validation)

            # get the keyword closet around
            if not closest_values:
                closest_values = find_closest_mate(file_keywords[0], values_key, no_of_mate=1,
                                                   field_validation=field_validation)
