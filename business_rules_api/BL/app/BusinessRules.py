...
class BusinessRules():
    ...
    def evaluate_rule(self,rule):
        logging.info(f"Evaluating the rule {rule}")
    ...
    def evaluate_condition(self, evaluations):
        for each_if_conditions in evaluations:
            ...
            logging.info(f'conditions got are \n{conditions}')
            logging.info(f'executions got are \n{executions}')
    ...
    def get_param_value(self, param_object):
        """Return the param value from the given type
        ...
        Note:
            ...
            If the parameter is itself a rule evaluate the rule and returns the value.
        """
        logging.info(f"PARAM OBJECT IS {param_object}")
        ...
        if param_source == 'input_config':
            ...
            logging.debug(f"table is {table_key} and column key is {column_key}")
            try:
                data = {}
                # update params data
                ...
                try:
                    ...
                    if column_key == 'Name':
                        data['value'] = {}
                        data['value']['Service'] = self.data_source[table_key]['Service_Provider_Name']
                        data['value']['Requesting'] = self.data_source[table_key]['Requesting_Provider_name']

                self.params_data['input'].append(data)
                print ("ADDing to paramas data")
                print (self.params_data)
                return data['value']
            except Exception as e:
                logging.error(f"table or column key not found")
                logging.error(str(e))
                logging.info(f"table data is {self.data_source}")
