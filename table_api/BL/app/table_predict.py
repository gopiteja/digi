...
def split_lines(L,R,T,B,ocrDataColumn):
    ...
    while(i<len(ocrDataColumn)-1):
        ...
        if((data['top']>=data2['top'] and data['top'] <=data2['bottom']) or (data2['top']>=data['top'] and data2['top'] <=data['bottom'])):
        ...
        for i in range(len(list2)):
            ...
            while(f == True and idx<length):
                if list1[idx].lower() not in list2[i+idx]['word'].lower():
                    f = False
                idx += 1
            if (f == True and idx == length and list2[i+length-1]['top']>= headerBottom and list2[i+length-1]['bottom']<=footer):
                first = i
                last = i+length-1
        matched_ocr = []
...
''' new '''
def make_pseudo_rectangle(pseudo_header_list,header_column_wise_data,invoice_left,invoice_right, pseudo_y_axis, img_ = None):
    coords = []
    rect_area = []
    words = []
    ...
    for i,header in enumerate(header_list):
        ...
        for word in header_words:
            ...
            for idx,data in enumerate(line_words_data):
                ...
                if(match_score > 0.75 and data['left']>=last_boundary and data['right'] < right_boundary and data['taken'] == False):
                    one_word_coord_list.append(data)
            if one_word_coord_list:
                words_coord_list.append(one_word_coord_list)
        try:
            header_words_rectangles, line_words_data = make_word_rectangle(words_coord_list.copy(), last_boundary,header_base_point, line_words_data)
            # #pass#print()
            #pass#print('here3')
            ...
            for rectangle in header_words_rectangles:
                if(abs(rectangle['coords'][1] - last_boundary) < abs(header_words_rectangle['coords'][1] - last_boundary) and rectangle['coords'][1]>last_boundary):
...
def header_lines(table_img,rf,header_column_wise_data, headerTop, headerLeft, headerRight, headerBottom,footer,ocrData,header_list,header_data):
    ...
    vertical_lines = vertical_slider(table_img,rf,header_column_wise_data,ocrData,headerLeft,headerRight,headerBottom,headerTop,footer,header_list,word_space)
...
def predict_kv_subtable(ocrData,json_data):
    ...
    try:
        ...
        for needle_for_kv in needles_for_kv:
            ...
            vertical_lines_subtable_kv.append([[right_boundary_subtable_kv, top],[right_boundary_subtable_kv,final_footer]])
...
def predict_hf_subtable(ocrData,json_data):
    ...
    try:
        ...
        subtable_h_left,subtable_h_right = side_corrections(ocrData, subtable_header_left, subtable_header_right, subtable_header_bottom, subtable_header_top, bottom)

        # making  subtable header hf
        horizontal_lines_subtable_hf = []
        vertical_lines_subtable_hf = []

        horizontal_lines_subtable_hf.append([[subtable_h_left,subtable_header_top],[subtable_h_right,subtable_header_top]])
        horizontal_lines_subtable_hf.append([[subtable_h_left,subtable_header_bottom],[subtable_h_right,subtable_header_bottom]])
...
def get_footer_matrix(ocrData,key_words,header_column_wise_data,alias_names, img):
    vers = [int( (header_column_wise_data[i]['coords'][2] + header_column_wise_data[i+1]['coords'][1])*0.5 ) for i in range(len(header_column_wise_data)-1)]
...
def get_hor_kv_footer(needle_for_kv,haystack,img): # Same as kv subtable
    ...
    try:
        ...
        right_boundary_haystack = ocrDataLocal(top,key_value_break,max([word['right'] for word in haystack]),bottom,haystack)
...
def predict_maintable_footer(ocrData,json_data,idx,headerBottom,kv_subtable_top = None):
    ...
    try:
        ...
        if footer_word:
            line_item_footer,footer_bottom,footer_left,footer_right,footer_string = match_list(footer_word,ocr_data_footer_finder)
...
def predict_maintable(ocrData, ocr_data_old, horizontal_lines,vertical_lines, headerBottom, req_cols,alias_names,subheader_data_arrays,img_ = None):
    extracted_data_maintable = []
    #pass#print('alias_names',alias_names)
    try:
        intersection_points_maintable = find_intersections(horizontal_lines, vertical_lines)
        #pass#print('crossed find interections')
        extracted_data_maintable,row_bounds = complex_data_table_generator(ocrData, intersection_points_maintable,req_cols, horizontal_lines, vertical_lines, img_,'subheader', True)
...
def row_splits(ocrData,json_data,horizontal_lines,vertical_lines,line_item_lines):
    ...
    try:
        ...
        for k,v in json_data.items():
            ...
            if(v['split_lines'] != 'true' and v['split_lines'] != 'false' and v['split_lines'] != '' and v['split_lines'] != [] and k != 'h0'):
...
def nested_columns(ocrData,table_img, col_children_no,header_column_wise_data,horizontal_lines,vertical_lines,headerLeft,headerRight, main_table_bottom, img_ = None,rf = None):
    nested_flag = False
        ...
        for key,value in col_children_no.items():
            ...
            try:
                ...
                for k,v in value.items():
                    ...
                    main_header_rect =  header_column_wise_data_function(main_heading_words,main_words_data,inv_width)[0]
                    ...
                    sub_header_rects =  header_column_wise_data_function(sub_heading_words,line_words_data,inv_width)[0]
                    ...
                    split_vers = vertical_slider(table_img,rf, sub_header_rects, ocr_data_subheader, col_left, col_right, header_bottom, header_top, main_table_bottom, sub_heading_words,word_space, img_)
                    # img_ = cv2.imread(imgpath)
                    # img_ = img.copy()
                    # w,h,c = img_.shape
                    # rf = 670/int(h)
            ...
            for i in range(len(ocrData)):
                ...
                try:
                    ...
                    try:
                        ...
                        header_column_wise_data,pseudo_header_list =  header_column_wise_data_function(header_list,line_words_data,invoice_right, img_)

                        try:
                            pseudo_y_axis = int(lines[line_number][0]['top'])
                            #pass#print('pseuod y axis: ', pseudo_y_axis)
                            header_column_wise_data = make_pseudo_rectangle(pseudo_header_list,header_column_wise_data,invoice_left,invoice_right, pseudo_y_axis, img_)
                        except Exception as e:
                            logging.debug(f'Pseudo Rectangle Failed:{e}')


                        headerTop, headerLeft, headerRight, headerBottom = finding_header_boundaries(header_column_wise_data)
                       ...
                       #########################         Header Rectangles Debugging            ############################################
                        ...
                        for subheader_words in subheader_words_list:
                            subheader_data_arrays.append(find_subheaders(ocrData[i], subheader_words, headerBottom, invoice_left, invoice_right, main_table_bottom) )
                        ...
                        # cv2.waitKey()
                        # cv2.destroyAllWindows()
                        header_hors, header_vers = header_lines(table_img,rf,header_column_wise_data, headerTop, headerLeft, headerRight, headerBottom, main_table_bottom,ocrData[i],header_list,header_words_data)
                        ...
                        for footer_type,info in footers_dict.items():
                            for no in range(len(info)):
                                ...
                                if footer_type == 'Main Table Matrix':
                                    ...
                                    extracted_data_fm_footer,_ = get_footer_matrix_(foot_path,label,vers_x,header_column_wise_data,alias_names,img_)
                                    if display == 'true':
                                        extracted_data_footer.append(extracted_data_fm_footer)
                        ...
                        horizontal_lines,line_item_lines = get_horizontal_lines(table_img_hors,rf,ocrData[i],header_hors,header_vers,headerLeft,headerRight,headerBottom,main_table_bottom,ref_col)

                        horizontal_lines = row_splits(ocrData[i],json_data,horizontal_lines,vertical_lines,line_item_lines)
                        ...
                        horizontal_lines,vertical_lines,nested_flag = nested_columns(ocrData[i],table_img, col_children_no,header_column_wise_data,horizontal_lines,vertical_lines,headerLeft,headerRight, main_table_bottom, img_,rf)
                        horizontal_lines = sorted(horizontal_lines, key = lambda x: x[0][1])
                        vertical_lines = sorted(vertical_lines, key = lambda x: x[0][0])

                        '''Validations'''

                        header_data = get_header_data(header_vers,ocrData[i],headerTop,headerLeft,headerRight,headerBottom)

                        '''Column-nature validation'''
                        try:
                            horizontal_lines,vertical_lines = column_nature_validation(ocrData[i],json_data,header_data,vertical_lines,horizontal_lines,header_data,headerBottom,footer)
                        except Exception as e:
                            pass
                            # logger.error('column nature validation Failed :  {}'.format(e))
                        '''Suffix Validation'''
                        try:
                            horizontal_lines,vertical_lines = suffix_validation(ocrData[i],json_data,vertical_lines,horizontal_lines,header_data,headerBottom)
                        ...
                        '''Prefix Validation'''
                        try:
                            vertical_lines,horizontal_lines = prefix_validation(ocrData[i],json_data,vertical_lines,horizontal_lines,header_data,headerBottom)
                        ...
                        extracted_data_maintable = predict_maintable(ocrData[i], ocr_data_old, horizontal_lines,vertical_lines, headerBottom, req_cols,req_alias_names,subheader_data_arrays)
                        ...
                        field_extracted_list = field_mapping(ocrData[i], extracted_data_maintable, field_json_data, horizontal_lines, vertical_lines, header_bottom, main_table_bottom, ocr_img_copy)
