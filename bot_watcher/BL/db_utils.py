...
class DB(object):
    def __init__(self, database, host=os.environ['HOST_IP'], user='root', password=os.environ['LOCAL_DB_PASSWORD'], port='3306', tenant_id=None):
        """
        ...
        if host in ["extraction_db", "queue_db", "template_db", "table_db", "stats_db", "business_rules_db"]:
    ...
    def connect(self, max_retry=5):
        ...
        try:
            ...
            self.db_ = create_engine(config, connect_args={'connect_timeout': 2})
    ...
    def execute(self, query, database=None, **kwargs):
        ...
        try:
            ...
            logging.debug('Query does not have any value to return.')
            return True
