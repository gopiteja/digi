$(document).ready(function () {
    ...
    //dynamicUrl = 'http://192.168.0.117:5019';
    fieldSplits = 0;
                ...
                if (!clickedAdd) {
                    ...
                    if ((a > b || a == b) && nullCheck(areas)) {
                        ...
                        if (text_ == '') {
                            ...
                            toast(alertmsg + ' is empty crop again', 'error');
                        } else {
                            ...
                            if ($('.vndr-' + id + '-' + target).length == 0) {
                                ...
                                ht += '<div class=" headerBox ">'
                                ht += '<div class="input-field  mr-b-0 mr-t-0">'
                                ht += '<h6 class="mr-t-0 font">' + value_on_crop + '</h6>'
                        ...
                        if (inital_ct == 0) {
                            ...
                            hp = "<p class='heading_font heading_font-" + id + "-" + target + "' id='address_crop'>Please Crop Address</p>"
                        } else if (inital_ct == 2) {
                            hp = "<p class='heading_font heading_font-" + id + "-" + target + "' id='footer_crop'>Please Crop Footer</p>"
                        } else if(inital_ct ==3 && (a == b || a > b)) {
                            $('uniq-' + id + '-' + target).remove()
                            hp = '<div class="row pos_rl uniq-' + id + '-' + target + '">'
                            hp += '<div class=" headerBox ">'
                            hp += '<div class="input-field  mr-b-0 mr-t-0">'
                            hp += '<h6 class="mr-t-0 font">Unique Fields</h6>'
                            hp += '<button class="addUniqFields">Add</button>'
                            hp += '<div class="clear___"></div>'
                            hp += '</div>'
                            hp += '<div class="conditionsHere"></div>'
                            hp += '<div class="addUniqueFieldsHere"></div>'
                            hp += '</div>'
                            hp += '</div>'
    ...
    function displayUniqFields(cropss) {
        rt = ''
        for (var cp = 0; cp < cropss.length; cp++) {
            ...
            text_ = $.trim(text);
            rt += '<input type="text" value="'+text_+'" readonly class="input">'
        }
        $(".addUniqueFieldsHere").html(rt)
        console.log(cropss);
    }
    ...
    $("body").on("click", ".nextToFields", function () {
        u_fids = $(".addUniqueFieldsHere input");
        u = []
        for (var i = 0; i < u_fids.length; i++) {
            u.push(u_fids[i].value);
        }
    ...
    $("body").on("click", ".saveBtn", function () {
        ...
        if ($.trim(template_name) != "") {
            ...
            if (temp_check == -1) {
                $.confirm({
                    title: 'Alert',
                    content: "Are you sure want to continue with new vendor ?",
                    buttons: {
                        ok: function () {
    ...
    function addFields(id, target, field_name, class_name_field, count, predicted_obj) {
        ...
        tr += '<p class="keywordSelect keyword-' + id + '-' + target + '">' + field_name + '</p>'
        tr += '</div>'
