...
def calculate_width_height(word, coords):
    ...
    if len(word) == 1:
        ...
        if coords[-1][0] == "\n":
            width = float(coords[-2][1]['r'])-float(coords[0][1]['l'])
            ##print ("NEWLINE WIDTH "*20)
            ##print (width)
            ##print ("NEWLINE WIDTH "*20)
            return {"word":word.replace('"','').replace("'",''), "width":float(coords[-2][1]['r'])-float(coords[0][1]['l']), "height":float(coords[-2][1]['b'])-float(coords[0][1]['t']),
            "top":float(coords[0][1]['t']), "left":float(coords[0][1]['l']),
            "bottom":float(coords[-1][1]['b']), "right":float(coords[-1][1]['r']),"confidence":float(confidence)}

        width = float(coords[-1][1]['r'])-float(coords[0][1]['l'])

        # #print ("WIDTH "*20)
        # #print (width)
        # #print ("WIDTH "*20)

        return {"word":word.replace('"','').replace("'",''), "width":float(coords[-1][1]['r'])-float(coords[0][1]['l']), "height":float(coords[-1][1]['b'])-float(coords[0][1]['t']),
        "top":float(coords[0][1]['t']), "left":float(coords[0][1]['l']),
        "bottom":float(coords[-1][1]['b']), "right":float(coords[-1][1]['r']),"confidence":float(confidence)}
