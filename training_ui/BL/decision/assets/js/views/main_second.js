...
(function(_, joint) {
    ...
    App.MainView = joint.mvc.View.extend({
        ...
        initializePaper: function() {
            ...
            paper.on('blank:pointerdown', _.partial(this.clicked, null), this);
            paper.on('cell:pointerdown', this.clicked, this);
            paper.on('cell:', this.clicked, this);
        ...
        createInspector: function(cell) {
            localStorage.setItem("selected", cell.id);
        ...
        initializeToolbar: function() {
            ...
            toolbar.on({
                ...
                'json:pointerclick': _.bind(this.openAsJson, this),
        ...
        exportStylesheet: '.scalable * { vector-effect: non-scaling-stroke }',
            openAsJson:function(){
                var cells_ = this.graph.toJSON().cells;
                var ruleBox = localStorage.getItem('ruleBox')
                var rules_ = ruleBox == undefined ? {} : JSON.parse(ruleBox)
                for (var i = 0; i < cells_.length; i++) {
                    if(rules_[cells_[i].id] != undefined) {
                        cells_[i].attrs.body.ruleCheck = JSON.stringify(rules_[cells_[i].id]).replace(/"/g, "'")
                    }
                }

                var obj_cell={};
                obj_cell.cells  =cells_
                console.log(JSON.stringify(obj_cell))
                // app.graph.fromJSON(obj_cell);
                // $(".joint-inspector").html('')
            },
        ...
        clicked: function (cellView, evt, x, y, delta) {

            if (cellView) {
                localStorage.setItem("selected", cellView.model.id);
                var ruleB = JSON.parse(localStorage.getItem('ruleBox'))
                if (ruleB[cellView.model.id] != undefined) {
                    $(".removeThis").remove()
                    setTimeout(function () {
                        $(".joint-inspector").append('<div class="removeThis">'+JSON.stringify(ruleB[cellView.model.id])+'</div>')
                    }, 400);
                }
            }
